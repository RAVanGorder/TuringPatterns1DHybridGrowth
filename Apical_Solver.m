...
% Function produces the plots for the apical growths (solve the PDE & extract numerical modes & solve the linear instability system)

%% Parameters
% Parameters for reaction-diffusion system
a=0.01;
b=1;
d_1 = 0.004;
d_2 = 0.1;
...
% Inital conditions for linear ODE system governing instability
...
%% Plot the solution, u, of the activator in the reaction-diffusion system
...
%% Solve the linear instability system, a system of ODEs

J = [(b-a)/(a+b), (a+b)^2; -2*b/(a+b), -(a+b)^2];
Dif = [d_1, 0; 0, d_2];
[time_modes,curr_sol] = ode45(@(t,kvec) derivModes(t,kvec,J,Dif,kappa,L), [0,Tf], IC1);

% Convert the ODE solutions into relative magnitudes over time
...
% Prescribed domain growth (and rates of change)
function z = r(t)
    z = 1+0.05*t;
end
