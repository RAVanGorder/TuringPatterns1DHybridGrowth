function Hybrid_Solver_Function_FHN(node_initals, s_dots, S_ints, final_time, time_res, space_res, max_k)
...
function zvec = kenetics(w) % FitzHugh-Nagumo reaction kenetics
    zvec = [c*(w(1)-(w(1)^3)/3+w(2)-i_0); (a-w(1)-b*w(2))/c];
end

% Jacobian matrix of rection kenetics
function M = J_kenetics(w)
    M = [c*(1-w(1)^2), c; -1/c,-b/c];
end

% Important quantity for homogeneous equilibria
U_1 = fzero(@(u) u-u^3/3+(a-u)/b-i_0,1);
